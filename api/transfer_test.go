...
func TestTransferAPI(t *testing.T) {
	...
	account1 := randomAccountWithParameter(user1.Username)
	account2 := randomAccountWithParameter(user2.Username)
	account3 := randomAccountWithParameter(user3.Username)

	account1.Currency = util.USD
	account2.Currency = util.USD
	account3.Currency = util.EURO
	...
	testCases := []struct {
		...
		},
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().GetAccount(gomock.Any(), gomock.Eq(account1.ID)).Times(1).Return(account1, nil)
				store.EXPECT().GetAccount(gomock.Any(), gomock.Eq(account2.ID)).Times(0) // Ensure no call for account2
